...
export default async function ProjectsPage() {
    ...
    return (
        <div className="min-h-screen bg-black text-white p-4 md:p-8 font-display pb-24">
                ...
                {projects && projects.length > 0 ? (
                    projects.map((project) => (
                        <Link key={project.id} href={`/projects/${project.id}`} className="block transition-transform active:scale-[0.98]">
                            <div className="bg-[#111] p-6 rounded-3xl border border-slate-800 shadow-sm hover:border-primary/50 transition-colors relative overflow-hidden">
                                ...
                                <p className="text-sm text-slate-400 line-clamp-2 ml-2">{project.description}</p>
                            </div>
                        </Link>
                    ))
                ) : (
                    <div className="p-12 text-center bg-[#111] rounded-3xl border border-slate-800 text-slate-500">
                        <Folder size={48} className="mx-auto mb-4 opacity-20" />
                        <h3 className="text-lg font-black text-slate-300 mb-1 uppercase tracking-widest">The Ledger is Empty</h3>
                        <p className="text-sm font-medium">All projects are yawped. Get back to work.</p>
                    </div>
                )}
            </div>
        </div>
